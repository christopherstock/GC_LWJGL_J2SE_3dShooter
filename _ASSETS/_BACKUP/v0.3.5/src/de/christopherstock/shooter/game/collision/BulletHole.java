/*  $Id: BulletHole.java 794 2011-05-27 22:46:10Z jenetic.bytemare $
 *  =======================================================================================
 */
    ...
    **************************************************************************************/
    public class BulletHole
    {
        ...
        {
            iHitPointX          = hitPoint.iVertex.x;
            iHitPointY          = hitPoint.iVertex.y;
            iHitPointZ          = hitPoint.iVertex.z;
            iHorzFaceAngle      = hitPoint.iHorzFaceAngle;
            iVertFaceAngle      = hitPoint.iVertFaceAngle;
            iCarrier            = hitPoint.iCarrier;
            iTexture            = hitPoint.iTexture;
            iBulletHoleSize     = hitPoint.iHoleSize;
...
//ShooterDebug.bugfix.out( "Vert face angle set to [" + iVertFaceAngle + "]" );
            ...
            float distX                 = -distanceHorzFromFace * -LibMath.cosDeg( hitPoint.iHorzFaceAngle );
            float distY                 =  distanceHorzFromFace *  LibMath.sinDeg( hitPoint.iHorzFaceAngle );
            float distZ                 = -distanceVertFromFace *  LibMath.sinDeg( hitPoint.iVertFaceAngle );

            //get the two holes that hit the wall from both sides
            Point2D.Float pointHorz1 = new Point2D.Float( hitPoint.iVertex.x + distX, hitPoint.iVertex.y + distY );
            Point2D.Float pointHorz2 = new Point2D.Float( hitPoint.iVertex.x - distX, hitPoint.iVertex.y - distY );
            ShooterDebug.bullethole.out( "bullet-hole: ["+pointHorz1.x+"]["+pointHorz1.y+"] ["+pointHorz2.x+"]["+pointHorz2.y+"]" );

            Point2D.Float pointVert1 = new Point2D.Float( hitPoint.iSrcPointVert.x, hitPoint.iVertex.z - distZ );
            Point2D.Float pointVert2 = new Point2D.Float( hitPoint.iSrcPointVert.x, hitPoint.iVertex.z + distZ );

            //select the nearer point
            float distanceHorz1 = (float)hitPoint.iSrcPointHorz.distance( pointHorz1 );
            float distanceHorz2 = (float)hitPoint.iSrcPointHorz.distance( pointHorz2 );
            Point2D.Float nearerHolePointHorz = ( distanceHorz1 < distanceHorz2 ? pointHorz1 : pointHorz2 );

            float distanceVert1 = (float)hitPoint.iSrcPointVert.distance( pointVert1 );
            float distanceVert2 = (float)hitPoint.iSrcPointVert.distance( pointVert2 );
/*
