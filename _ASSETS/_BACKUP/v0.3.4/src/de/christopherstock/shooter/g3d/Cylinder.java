/*  $Id: Cylinder.java 706 2011-05-04 21:40:34Z jenetic.bytemare $
 *  =======================================================================================
 */
    ...
    **************************************************************************************/
    public class Cylinder implements LibGeomObject
    {
        ...
        public Cylinder( GameObject aParentGameObject, LibVertex aAnchor, float aRadius, float aHeight, int aCollisionCheckingSteps, LibDebug aDebug, boolean aDebugDrawBotCircles, float aBottomCollisionToleranceZ, float aMinBottomCollisionToleranceZ )
        {
            anchor                          = aAnchor;
...
//                iDebug.out( ">> SLIVER-ANGLE-VERT: " + SliverAngleVert );
                ...
                (
                    parentGameObject,
                    ShooterTexture.getBulletHoleTextureForMaterial( LibGLMaterial.EUndefined ).getTexture(),
                    ShooterTexture.getSliverColorsForMaterial(      LibGLMaterial.EUndefined ),
        ...
        ************************************************************************************/
        public final boolean heightsIntersect( float wallZ1, float wallZ2, boolean useBottomToleranceZ )
        {
            ...
            boolean miss =
            (
                    anchor.z + ( useBottomToleranceZ ? iBottomCollisionToleranceZ : iMinBottomCollisionToleranceZ ) > highestWallZ
                ||  anchor.z + height                                                                               < lowestWallZ
            );
        ...
        **************************************************************************************/
        ...
        public final boolean checkCollisionHorzLines( FaceTriangle face, boolean actionRadius, boolean useBottomToleranceZ )
        {
            //cylinders will not collide on floors
            //if ( face.iLowestZ == face.iHighestZ ) return false;

            //check horizontal intersection
            Point2D.Double  cylinderPosHorz         = new Point2D.Double( anchor.x, anchor.y );
            boolean         horizontalIntersection1 = face.iCollisionLineHorz1.ptSegDist( cylinderPosHorz ) <= ( actionRadius ? getActionRadius() : getRadius() );
            boolean         horizontalIntersection2 = face.iCollisionLineHorz2.ptSegDist( cylinderPosHorz ) <= ( actionRadius ? getActionRadius() : getRadius() );
            boolean         horizontalIntersection3 = face.iCollisionLineHorz3.ptSegDist( cylinderPosHorz ) <= ( actionRadius ? getActionRadius() : getRadius() );

            if ( horizontalIntersection1 || horizontalIntersection2 || horizontalIntersection3 )
            {
                //check vertical intersection
                boolean verticalIntersection = heightsIntersect( face.iLowestZ, face.iHighestZ, useBottomToleranceZ );
        ...
        **************************************************************************************/
        ...
        public final boolean checkCollisionHorz( Cylinder c )
        {
            ...
            if ( checkCollision( c.getCircle() ) && c.heightsIntersect( anchor.z, anchor.z + height, false ) )
            {
