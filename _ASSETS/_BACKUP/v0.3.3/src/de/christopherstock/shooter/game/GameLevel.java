/*  $Id: GameLevel.java 633 2011-04-23 23:33:09Z jenetic.bytemare $
 *  =======================================================================================
 */
    ...
    **************************************************************************************/
    public class GameLevel
    {
        ...
        **************************************************************************************/
        ...
        private GameLevel( LevelConfig aConfig )
        {
            iConfig = aConfig;
            iPlayer = new Player
            (
...
/*
            Bot b0 = new Bot(  ShooterBotTemplate.ESecurityMale,        Bot.BotType.ETypeFriend, new LibVertex( 7.5f, 6.5f, 0.0f + 0.001f ),  wayPointsEnemy1, BotJob.ELeadPlayerToLastWaypoint,    Bots.EItemPistol,     null                );
...
*/
        ...
        {
            if ( GameLevel.levelToChangeTo != null )
            {
                if ( levelChangeBlocker <= System.currentTimeMillis() )
                {
                    //change to if possible
                    if ( currentLevel != null && GameLevel.levelToChangeTo == currentLevel.iConfig )
                    {
                        ShooterDebug.levels.out( "deny change to equal level [" + GameLevel.levelToChangeTo + "]" );
                    }
                    else
                    {
                        ShooterDebug.levels.out( "change level to [" + GameLevel.levelToChangeTo + "]" );

                        new HUDMessage( "Push ENTER to get some hints" ).show();

                        //change current level
                        currentLevel = new GameLevel( GameLevel.levelToChangeTo );
                    ...
                    GameLevel.levelToChangeTo = null;
                }
            }
        }
...
*/
        public final Float getHighestFloor( Point2D.Float point )
        {
            ...
            //return nearest floor
            return GameLevel.currentPlayer().getCylinder().getHighestFloor( hitPointsZ, PlayerAttributes.MAX_CLIMBING_UP_Z );
        }
