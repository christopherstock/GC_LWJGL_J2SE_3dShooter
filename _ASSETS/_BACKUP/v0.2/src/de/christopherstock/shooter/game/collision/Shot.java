    ...
    **************************************************************************************/
    public class Shot
    {
        ...
        public Shot( ShotOrigin aOrigin, boolean aViewOnly, float irregularityAngle, float irregularityDepth, float startX, float startY, float startZ, float aRotZ, float aRotX, float aShotRange, BulletHole.PointSize aBulletHoleSize )
        {
            ...
            viewOnly        = aViewOnly;
            origin          = aOrigin;
            srcPoint        = new Vertex( startX, startY, startZ );
            rotZ            = aRotZ;
            rotX            = aRotX;
            ...
            (
                srcPointHorz.x - UMath.sinDeg( rotZ ) * shotRange,
                srcPointHorz.y - UMath.cosDeg( rotZ ) * shotRange
            );
            ...
            (
                srcPointVert.x + UMath.cosDeg( rotX ) * shotRange,
                srcPointVert.y - UMath.sinDeg( rotX ) * shotRange
            );
        ...
        public HitPoint launch()
        {
            ...
            )
            {
                ...
                switch ( hitPoint.gameObject.getHitPointCarrier() )
                {
                    ...
                    {
                        //player loses health
                        Player.singleton.hurt( UMath.getRandom( 2, 10 ) );
                        break;
                    }
