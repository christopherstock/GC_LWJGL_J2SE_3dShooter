    ...
    package de.christopherstock.shooter.temp;
    ...
    //setSpotLight us very interesting ..
    ...
    public class GLTempApp {
        ...
        public static ByteBuffer    tmpInt = allocBytes(GLTempApp.SIZE_INT); // temp var used by getPixelColor()
        ...
        /**
         * Called by run() to draw one frame.  Subclass will override it.
        ...
        {
            int textureHandle = 0;
            GLTempImage textureImg = loadImage(textureImagePath);
            if (textureImg != null) {
         ...
         */
        public static int makeTexture(GLTempImage textureImg)
        {
            if ( textureImg != null ) {
        ...
        {
            if (pixelsARGB != null) {
                ByteBuffer pixelsRGBA = GLTempImage.convertImagePixelsRGBA(pixelsARGB,w,h,true);
         ...
         */
        public static int makeTextureMipMap(int textureHandle, GLTempImage textureImg)
        {
            int ret = 0;
         ...
         */
        public static void copyImageToTexture(GLTempImage img, int textureHandle) {
        ...
        */
         ...
         */
        public static GLTempImage makeImage(int w, int h) {
            ByteBuffer pixels = allocBytes(w*h*SIZE_INT);
            return new GLTempImage(pixels,w,h);
        }
         ...
         */
        public static GLTempImage loadImage(String imgFilename) {
            GLTempImage img = new GLTempImage(imgFilename);
            if (img.isLoaded()) {
                return img;
            }
            return null;
         ...
         * @see resetViewport()
         */
        public static void drawImageFullScreen(GLTempImage img) {
         ...
         */
        public static void drawImage(GLTempImage img, int x, int y, float w, float h) {
            // if image has no texture, convert the image to a texture
            if (img.textureHandle <= 0) {
                img.textureHandle = makeTexture(img);
            }
        ...
        public static boolean buildFont(String charSetImage, int fontWidth)
        {
            ...
            GLTempImage textureImg = loadImage(charSetImage);
            if (textureImg == null) {
        ...
        public static void screenShot(int x, int y, int width, int height, String imageFilename) {
            ...
            GLTempImage.savePixelsToPNG(pixels, width, height, imageFilename, true);
        }
        ...
        public static void savePixelsToPNG(ByteBuffer framebytes, int width, int height, String imageFilename, boolean flipY) {
            if (framebytes != null && imageFilename != null) {
                ...
                framebytes.asIntBuffer().get(pixels, 0, pixels.length);
                // save pixels to file
                GLTempImage.savePixelsToPNG(pixels, width, height, imageFilename, flipY);
            }
        }
        ...
        public static void screenShotRGB(int width, int height, String saveFilename) {
            ...
            ByteBuffer framebytes = GLTempApp.allocBytes(width * height * 3);
            int[] pixels = new int[width * height];
            int bindex;
            ...
            GLTempImage.savePixelsToPNG(pixels, width, height, saveFilename, true);
        }
        ...
        public static Class rootClass = GLTempApp.class;
         ...
         * the folder tree.  Below we call getClass() to record the class of the
        ...
        public void invoke(Method method) {
            if (method != null){
                try {
                    ...
                    // Error handling
                    System.err.println("GLApp.invoke(): couldn't invoke method " + method.getName() + " on object " + getClass().getName());
                }
            }
        }
    }