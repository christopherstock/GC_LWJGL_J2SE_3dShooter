/*  $Id: Bot.java 1134 2012-03-12 19:51:02Z jenetic.bytemare $
 *  =======================================================================================
 */
    ...
    **************************************************************************************/
    public class Bot implements LibGameObject, ShotSpender
    {
        ...
        private                     ShooterBotPattern   iTemplate                   = null;
        ...
        public Bot( ShooterBotPattern aTemplate, BotType aType, LibVertex aStartPosition, Point2D.Float[] aWayPoints, BotJob aJob, Artefact[] aArtefacts, float aFacingAngle, BotAction[] aActions, BotHealth aBotHealth, int aID, BotHanded aBotHanded )
        {
            iType           = aType;
            iCylinder       = new Cylinder( this, aStartPosition, aTemplate.iBase.iRadius, aTemplate.iBase.iHeight, 0, ShooterDebug.bot, ShooterDebug.DEBUG_DRAW_BOT_CIRCLES, 0.0f, 0.0f );
            ...
            switch ( iHanded )
            {
                case ELeftHanded:
                {
                    leftItem = iArtefactSet.iCurrentArtefact.iArtefactType.iMesh;
                    break;
                }

                case ERightHanded:
                {
                    rightItem = iArtefactSet.iCurrentArtefact.iArtefactType.iMesh;
                    break;
                }
            }
        ...
        @Override
        public final void launchAction( Cylinder aCylinder, Gadget artefact, float faceAngle )
        {
            //only if alive
            if ( iAliveState == BotAliveState.EAlive )
            {
                ...
                {
                    float anglePlayerToWall = LibMath.getAngleCorrect( ShooterGameLevel.currentPlayer().getCylinder().getCenterHorz(), iCylinder.getCenterHorz() );
                            ...
                            if ( iActions != null )
                            {
                                for ( BotAction action : iActions )
                                {
                                    if ( action instanceof BotGiveAction )
                                    {
                                        ...
                                        if ( ga.iKey == artefact.iParentKind )
                                        {
                                            ga.iEvent.perform( this );
                                            ShooterGameLevel.currentPlayer().iArtefactSet.extractArtefact( artefact );
                                        }
                                    }
                                }
                            }
        ...
        protected final boolean checkCollisionsToPlayer()
        {
            ...
            if ( checkCollision( ShooterGameLevel.currentPlayer().getCylinder().getCircle() ) )
            {
                                ...
                                if ( Math.abs( angleBotToPlayer - iFacingAngle ) <= BotSettings.TARGET_TURNING_TOLERANCE )
                                {
                                    if ( iBotMeshes.isAssignedArmsPosition( ( iHanded == BotHanded.ELeftHanded ? ArmsPosition.EAimHighLeft : ArmsPosition.EAimHighRight ) ) )
                                    {
                                        ...
                                        )
                                        {
                                            ...
                                            if
                                            (
                                                    !ShooterGameLevel.currentPlayer().isDeadAnimationOver()
                                                &&  botSeesThePlayer()
                                            )
                                            {
                                                //fire
...
*/
        ...
        public final LibShot getShot( float modZ )
        {
            ...
            (
                ...
                getAnchor().x,
                getAnchor().y,
                getAnchor().z + PlayerAttributes.DEPTH_HAND_STANDING,   // take hand height from player
                180.0f + ( 180.0f - iFacingAngle ),     //rotZ
                0.0f,                                   //rotX
                BotSettings.SHOT_RANGE,
                LibHoleSize.E44mm,   // bot always fires 9mm    ??
                ShooterDebug.shotAndHit,
                ArtefactType.EMagnum357.iWearponBehaviour.getSliverParticleQuantity(),
                FxSettings.SLIVER_ANGLE_MOD,
                10,
                ArtefactType.EMagnum357.iWearponBehaviour.getSliverParticleSize(),
                ArtefactType.EMagnum357.getIsWallBreakable(),
                ArtefactType.EMagnum357.getProjectile()
            );
        }
        ...
        public final boolean botSeesThePlayer()
        {
            ...
            LibHitPoint[] hitPoint = ShooterGameLevel.current().launchShot( shot );
        ...
        public void makeDistancedSound( ShooterSound fx )
        {
        ...
        public void fallAsleep()
        {
            if ( iAliveState == BotAliveState.EAlive )
            {
                kill( BotAliveState.ETranquilized );
            }
        }
        ...
        public void hurt( int damage )
        {
            ...
            //bot dies?
            if ( iAliveState == BotAliveState.EAlive && iHealth <= 0 )
            {
                kill( BotAliveState.EDead );
            }
        }
        ...
        public void kill( BotAliveState newState )
        {
            //let bot die
            iHealth         = 0;
            ...
            //start disappear timer ( will only count for dead bots )
        ...
        public final void dropAllArtefacts()
        {
            ...
            for ( Artefact toDrop : iArtefactSet.iArtefacts )
            {
                ...
                if ( p != null )
                {
                    ...
                    p.iDropTarget = ShooterGameLevel.currentPlayer().getAnchor().z;
                    p.iDropBegin  = iBotMeshes.iEquippedItemRight.getCenterZ();
                    ShooterGameLevel.current().iItems.add( p );
                }
            }
