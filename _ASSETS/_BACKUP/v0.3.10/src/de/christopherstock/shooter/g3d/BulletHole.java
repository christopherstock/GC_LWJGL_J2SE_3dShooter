/*  $Id: BulletHole.java 1134 2012-03-12 19:51:02Z jenetic.bytemare $
 *  =======================================================================================
 */
    ...
    public class BulletHole
    {
        ...
        public BulletHole( LibHitPoint aHitPoint, D3dsFile aProjectile )
        {
            ...
            if ( iProjectileTemplate != null )
            {
                ...
                //only set once .. :/
                iProjectile = new Mesh( ShooterD3ds.getFaces( iProjectileTemplate ), iHitPoint.iVertex, 0.0f, 1.0f, Invert.ENo, iHitPoint.iCarrier, LibTransformationMode.EOriginalsToOriginals, DrawMethod.EHideIfTooDistant );
...
*/
        ...
        {
            //draw all points
            for ( BulletHole bulletHole : ShooterGameLevel.iBulletHoles )
            {
        ...
        public static final void removeForWall( Wall bhc )
        {
            ...
            for ( int i = ShooterGameLevel.iBulletHoles.size() - 1; i >= 0; --i )
            {
                //check if this bullet hole belongs to the specified mersh
                if ( ShooterGameLevel.iBulletHoles.elementAt( i ).iHitPoint.iCarrier == bhc )
                {
                    //remove this bullet hole
                    ShooterGameLevel.iBulletHoles.removeElementAt( i );
                }
            }
        }
        ...
        public static final void darkenAll( Wall bhc, float opacity )
        {
            ...
            for ( int i = ShooterGameLevel.iBulletHoles.size() - 1; i >= 0; --i )
            {
                ...
                {
                    //darken bulletHole
                    ShooterGameLevel.iBulletHoles.elementAt( i ).darken( opacity );
                }
            }
        }
        ...
        private final float getSuitableDistanceFromHorzFace()
        {
            ...
            for ( BulletHole bulletHole : ShooterGameLevel.iBulletHoles )
            {
                ...
                if ( new Point2D.Float( bulletHole.iHitPoint.iVertex.x, bulletHole.iHitPoint.iVertex.y ).distance( new Point2D.Float( iHitPoint.iVertex.x, iHitPoint.iVertex.y ) ) < MIN_POINT_DISTANCE_FOR_SAME_LAYER_MULTIPLIER * iHitPoint.iShot.iBulletHoleSize.size )
                {
        ...
        private final float getSuitableDistanceFromVertFace()
        {
            ...
            {
                //should be distance3d :/
                if ( Math.abs( bulletHole.iHitPoint.iVertex.z - iHitPoint.iVertex.z ) < MIN_POINT_DISTANCE_FOR_SAME_LAYER_MULTIPLIER * iHitPoint.iShot.iBulletHoleSize.size )
                {
                    dis += STEP_DISTANCE_FROM_FACE;
                }
            }
