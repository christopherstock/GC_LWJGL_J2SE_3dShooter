/*  $Id: BotMeshes.java 542 2011-04-13 14:47:36Z jenetic.bytemare $
 *  =======================================================================================
 */
        ...
        **************************************************************************************/
        public BotMeshes( ShooterBotTemplate aTemplate, LibVertex aAnchor, GameObject aParentGameObject, Bots itemLeft, Bots itemRight )
        {
            ...
            else
            {
                ...
                iHat.changeTexture(             ShooterTexture.ETest.iTexture,          aTemplate.iTexHat.iTexture                  );
            }
            ...
            else
            {
                ...
                iGlasses.changeTexture(             ShooterTexture.ETest.iTexture,          aTemplate.iTexGlassesGlass.iTexture         );
                iGlasses.changeTexture(             ShooterTexture.EHairBlonde.iTexture,    aTemplate.iTexGlassesHolder.iTexture        );
            }
            ...
            iRightUpperLeg.changeTexture(   ShooterTexture.ETest.iTexture,          aTemplate.iTexRightUpperLeg.iTexture        );
            iLeftUpperLeg.changeTexture(    ShooterTexture.ETest.iTexture,          aTemplate.iTexLeftUpperLeg.iTexture         );
            iRightLowerLeg.changeTexture(   ShooterTexture.ETest.iTexture,          aTemplate.iTexRightLowerLeg.iTexture        );
            iLeftLowerLeg.changeTexture(    ShooterTexture.ETest.iTexture,          aTemplate.iTexLeftLowerLeg.iTexture         );

            iLeftFoot.changeTexture(        ShooterTexture.ETest.iTexture,          aTemplate.iTexLeftFoot.iTexture             );
            iRightFoot.changeTexture(       ShooterTexture.ETest.iTexture,          aTemplate.iTexRightFoot.iTexture            );
            iLeftUpperArm.changeTexture(    ShooterTexture.ETest.iTexture,          aTemplate.iTexLeftUpperArm.iTexture         );
            iRightUpperArm.changeTexture(   ShooterTexture.ETest.iTexture,          aTemplate.iTexRightUpperArm.iTexture        );
            iLeftLowerArm.changeTexture(    ShooterTexture.ETest.iTexture,          aTemplate.iTexLeftLowerArm.iTexture         );
            iRightLowerArm.changeTexture(   ShooterTexture.ETest.iTexture,          aTemplate.iTexRightLowerArm.iTexture        );
            iLeftHand.changeTexture(        ShooterTexture.ETest.iTexture,          aTemplate.iTexLeftHand.iTexture             );
            iRightHand.changeTexture(       ShooterTexture.ETest.iTexture,          aTemplate.iTexRightHand.iTexture            );
            iNeck.changeTexture(            ShooterTexture.ETest.iTexture,          aTemplate.iTexNeck.iTexture                 );
            iTorso.changeTexture(           ShooterTexture.ETest.iTexture,          aTemplate.iTexTorsoUpper.iTexture           );
            iTorso.changeTexture(           ShooterTexture.EHairBlonde.iTexture,    aTemplate.iTexTorsoLower.iTexture           );
            iHead.changeTexture(            ShooterTexture.EHairBlonde.iTexture,    aTemplate.iTexHair.iTexture                 );
            iFace.changeTexture(            ShooterTexture.ETest.iTexture,          aTemplate.iTexFaceEyesOpen.iTexture         );
        ...
        public void test()
        {
            //iHead.changeTexture( ShooterTexture.EFaceFemale1.iTexture, ShooterTexture.EFace3.iTexture );
        }

        public void test2()
        {
            //iHead.changeTexture( ShooterTexture.EFaceFemale2.iTexture, ShooterTexture.EFace2.iTexture );
        }
        ...
        private Rotation[] getNewArmPosition( Arm arm, ArmTarget newArmPosition )
        {
            ...
            switch ( newArmPosition )
            {
                case ERestInHip:
                {
                    upperArmPitch.set( 0.0f, 0.0f,   0.0f  );
                    lowerArmPitch.set( 0.0f, -10.0f, 0.0f );
                    handPitch.set(     0.0f, 0.0f,   0.0f  );
                    break;
                }

                case EPickUp:
                {
                    upperArmPitch.set(  -50.0f, 0.0f,  0.0f );
                    lowerArmPitch.set(  -50.0f, 0.0f,  0.0f );
                    handPitch.set(       -55.0f, 35.0f, 0.0f );
                    break;
                }

                case EPointToSide:
                {
                    upperArmPitch.set(  0.0f, -50.0f,  0.0f );
                    lowerArmPitch.set(  0.0f, -110.0f, 0.0f );
                    handPitch.set(       0.0f, -95.0f,  0.0f );
                    break;
                }

                case EAimLow:
                {
                    upperArmPitch.set(  -50.0f, 50.0f, 0.0f );
                    lowerArmPitch.set(  -60.0f, 0.0f,  0.0f );
                    handPitch.set(       -95.0f, 0.0f,  0.0f );
                    break;
                }

                case EEllbowBack:
                {
                    upperArmPitch.set(  0.0f,   0.0f,  90.0f );
                    lowerArmPitch.set(  0.0f,   40.0f, 90.0f );
                    handPitch.set(       -55.0f, 0.0f,  0.0f  );
                    break;
                }

                case EAimHigh:
                {
                    upperArmPitch.set(  -90.0f, 0.0f,  0.0f  );
                    lowerArmPitch.set(  -90.0f, 0.0f,  0.0f  );
                    handPitch.set(      -90.0f, -90.0f,  0.0f );
                    break;
                }

                case EDown:
                {
                    upperArmPitch.set(  0.0f, 20.0f,  0.0f );
                    lowerArmPitch.set(  0.0f, -25.0f, 0.0f );
                    handPitch.set(       0.0f, -10.0f, 0.0f );
                ...
                {
                    upperArmPitch.set(   0.0f,   -120.0f, 0.0f );
                    lowerArmPitch.set(   0.0f,   120.0f,  0.0f );
                    handPitch.set(       185.0f, 0.0f,    0.0f );
                    break;
                }
            }
        ...
        private void assignHeadPosition( HeadPosition newHeadPosition )
        {
        ...
        private final Rotation[] getNewLegPosition( Leg leg, LegTarget newLegPosition )
        {
            ...
            switch ( newLegPosition )
            {
                ...
                {
                    upperLegPitch.set(  0.0f,   0.0f,   0.0f );
                    lowerLegPitch.set(  0.0f,   0.0f,   0.0f );
                    footPitch.set(      0.0f,   0.0f,   0.0f );
                    break;
                }

                case EKicking:
                {
                    upperLegPitch.set(  -90.0f,   0.0f,   0.0f );
                    lowerLegPitch.set(  -90.0f,   0.0f,   0.0f );
                    footPitch.set(      -90.0f,   0.0f,   0.0f );
                    break;
                }

                case EKickingHigh:
                {
                    upperLegPitch.set(  -135.0f,   0.0f,   0.0f );
                    lowerLegPitch.set(  -135.0f,   0.0f,   0.0f );
                    footPitch.set(      -135.0f,   0.0f,   0.0f );
                    break;
                }
            }
            ...
            switch ( leg )
            {
                ...
                {
                    //flip y axis for left leg!
                    upperLegPitch.set(   upperLegPitch.x, -upperLegPitch.y, -upperLegPitch.z );
                    lowerLegPitch.set(   lowerLegPitch.x, -lowerLegPitch.y, -lowerLegPitch.z );
                    footPitch.set(       footPitch.x,     -footPitch.y,     -footPitch.z     );
                    break;
                }
        ...
        public void transformLimbs( LibVertex botAnchor, float facingAngle )
        {
            //set target pitches
            setTargetPitches();
            ...
            turnAllLimbsZ( botAnchor, facingAngle );
        }
...
//            iFace.setPitch( iDebugHeadTurn, 0.0f, 0.0f );
            ...
            if ( h )
            {
                ...
                if ( currentTargetPitchHead >= iHead.targetPitch.size() ) currentTargetPitchHead = 0;
            }
            ...
            if ( ra1 & ra2 & ra3 )
            {
                ...
                if ( currentTargetPitchRightArm >= iRightUpperArm.targetPitch.size() ) currentTargetPitchRightArm = 0;
            }
            ...
            if ( la1 & la2 & la3 )
            {
                ...
                if ( currentTargetPitchLeftArm >= iLeftUpperArm.targetPitch.size() ) currentTargetPitchLeftArm = 0;
            }
            ...
            if ( rl1 & rl2 & rl3 )
            {
                ...
                if ( currentTargetPitchRightLeg >= iRightUpperLeg.targetPitch.size() ) currentTargetPitchRightLeg = 0;
            }
            ...
            if ( ll1 & ll2 & ll3 )
            {
                ...
                if ( currentTargetPitchLeftLeg >= iLeftUpperLeg.targetPitch.size() ) currentTargetPitchLeftLeg = 0;
            }
        }
